...
class Statistic
{
    ...
    public function productDefectiveReportByLine($request)
    {
        ...
        $productDefective = $productDefective->rightJoin('Master_Machine', function($join) use ($start, $end) {
            $join->on('Product_Defective_Log.Master_Machine_ID', '=', 'Master_Machine.ID')
                ->where('Product_Defective_Log.Time_Created', '>=', $start->add(8, 'hours'))
                ->where('Product_Defective_Log.Time_Updated', '<=', $end->add(1, 'day')->add(8, 'hours')->sub(1, 'second'));
        })
        ->leftJoin('Production_Log', 'Product_Defective_Log.Production_Log_ID', '=', 'Production_Log.ID')
        ->leftJoin('Master_Status', 'Production_Log.Master_Status_ID', '=', 'Master_Status.ID')
        ->where('Master_Machine.Line_ID', '=', $lineId)
        ->select(
            'Product_Defective_Log.Quantity',
            'Product_Defective_Log.Time_Created',
            'Master_Machine.Name as Machine_Name',
            'Master_Status.Name as Error_Name',
            'Master_Status.ID as Error_ID'
        );
        ...
        foreach ($category as $machineName) {
            ...
            $changeMold[] = $items->has('SHORT MOLD') ? $items['SHORT MOLD'] : '';
            $burr[] = $items->has('BURR') ? $items['BURR'] : '';
            $dim[] = $items->has('DIM') ? $items['DIM'] : '';
            $shape[] = $items->has('SHAPE CHANGE') ? $items['SHAPE CHANGE'] : '';
            $IBUTSU[] = $items->has('IBUTSU') ? $items['IBUTSU'] : '';
            $others[] = $items->has('Others') ? $items['Others'] : '';
        }
        return [
            'chartBarData' => [
                'category' => $category,
